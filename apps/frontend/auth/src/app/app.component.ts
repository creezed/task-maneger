...
@Component({
    ...
    template: `
        <header class="flex flex-row justify-content-between align-items-center h-3rem">
            <nav class="flex gap-3">
                <a class="text-color-secondary text-lg no-underline hover:text-primary" [routerLink]="[Routes.LOGIN]"
                    >Вход</a
                >
                <a
                ...
                <span
                    >Статус
                    <span class="text-primary">{{ (isAuth$ | async) ? 'Авторизован' : 'Не авторизован' }}</span></span
                >
                <span *ngIf="isAuth$ | async" class="cursor-pointer" (click)="authService.logout()">Выход</span>
            </nav>
        </header>
